...
namespace ml
{
  ...
  namespace ann
  {
    ...
    //                                                                                    w00
    // x0         +-> |[X] - [C0]| --> phi(|[X]-[C0]|) = exp(-|[X]-[C0]|^2/(2*sigma_0^2)) ----->y_0
    //           /                                                                       w01 // w02
    // x1 = [X] +---> |[X] - [C1]| --> phi(|[X]-[C1]|) = exp(-|[X]-[C1]|^2/(2*sigma_1^2)) -+ /
    //           \                                                                          /
    // x2         +-> |[X] - [C2]| --> phi(|[X]-[C2]|) = exp(-|[X]-[C2]|^2/(2*sigma_2^2)) -+  -> y_1
    //
    // x3
    //
    // [Cj] = [cj0, cj1, cj2, cj3]
    ...
    //
    // Nk = Ninputs = 4. Here Ni.
    // Nj = Nhidden = 3. Here Nh.
    // Ni = Noutput = 2. Here No.
    class RBFNetwork
    {
      const size_t Ni = 0;
      const size_t Nh = 0;
    ...
    public:
                                                      ...
                                                      float eta = 0.1f, float mu = 0.05f, float r = 0.f)
      {
        ...
        for (size_t o_idx = 0; o_idx < No; ++o_idx)
        {
          ...
          for (size_t i_idx = 0; i_idx < Ni; ++i_idx)
            dy_dw[i_idx] = 2*(inputs[i_idx].get().value_or(nan) - w_o[i_idx]);
          auto dC_dw = stlutils::mult_scalar(dy_dw, dC_dy);
